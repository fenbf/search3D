...
{
    public class YahooImage : ISearchElement
    {
        public string url;
    ...
    public class YahooEngine : ISearchEngine
    {
        ...
        private Queue<YahooImage> imagesToLoad;
        ...
        public YahooEngine() 
        {
            imagesToLoad = new Queue<YahooImage>();
            pattern = null;
        ...
        public void SearchNext(System.ComponentModel.BackgroundWorker bw) 
        {
            if (start < maxResult)
            {
                try
                {
                    ...
                    foreach (Yahoo.API.ImageSearchResponse.ResultType result in resultSet.Result)
                    {
                        ...
                        YahooImage img = new YahooImage();
                        img.url = result.Thumbnail.Url;
                        img.urlBig = result.Url;
            ...
            while (counter < maxRoundCount && imagesToLoad.Count > 0) 
            {
                ...
                YahooImage img = imagesToLoad.Dequeue();
                if (img.Load() == true)
                {
